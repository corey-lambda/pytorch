...
def _cpp_cia(*args, **kwargs):
    kernel = kwargs["kernel"]
    python_cia = kwargs["python_cia"]
    del kwargs["kernel"]
    del kwargs["python_cia"]

    if "matmul" in str(kernel):
        return python_cia(*args, **kwargs)
...
def _override_composite_implicit_decomp(cia_ops_to_callable, safe=True):
    ...
    for op_overload, decomp_callable in cia_ops_to_callable.items():
        ...
        if (
            torch._C.DispatchKey.Meta not in op_overload.py_kernels
        ):  
            ...
            op_overload.py_impl(torch._C.DispatchKey.Meta)(
                functools.partial(orig_cia_callable)
            )
